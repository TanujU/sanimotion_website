/*
 * /orthopaedische-einlagen — Orthopedic insoles landing page.
 *
 * Mirrors sanimotion.com/orthopaedische-einlagen-berlin/: hero (split with
 * photo) → intro + Schuhtypen list → Krankenkasse → 6 service highlights →
 * Fußabdruck (with photo) → Maßanfertigung detail blocks → contact.
 */
"use client";
import type { Route } from "./+types/orthopaedische-einlagen";
...
import { getEinlagenContent } from "~/content/pages/orthopaedische-einlagen";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/einlagen/einlagen-detail.jpeg";
import detailImageUrl from "~/images/einlagen/einlagen-hero.jpeg";

export function meta() {
  const c = getEinlagenContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/orthopaedische-einlagen" });
}

export default function OrthopaedischeEinlagen() {
  const locale = useLocale();
  const c = getEinlagenContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Orthopädische Einlagen", path: "/orthopaedische-einlagen" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/orthopaedische-einlagen" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Produkte", path: "/#produkte" },
        { name: "Orthopädische Einlagen", path: "/orthopaedische-einlagen" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Orthopädische Einlagen aus der Sanimotion-Werkstatt"
          ...
          <div className="mx-auto max-w-[60ch] space-y-8">
            ...
            <Reveal delay={0.05}>
              <p className="text-body-lg text-ink-muted">
                {c.hero.schuhtypenIntro}
              </p>
            </Reveal>
            <Reveal delay={0.1}>
              <ul className="grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2">
                {c.hero.schuhtypen.map((typ) => (
                  <li
                    key={typ}
      ...
      <Section tone="muted">
        <Container>
          <div className="grid items-start gap-12 lg:grid-cols-12 lg:gap-16">
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Fußabdruck im Sanimotion-Sanitätshaus"
            ...
            <div className="lg:col-span-7">
              <Reveal>
                <Eyebrow>{c.fussabdruck.eyebrow}</Eyebrow>
                <Heading
                  as="h2"
                  size="display-md"
              ...
              <div className="mt-8 space-y-6">
                {c.fussabdruck.paragraphs.map((p, i) => (
                  <Reveal key={i} delay={0.05 + i * 0.05}>
      ...
      <Section tone="canvas">
        <Container>
          ...
          <div className="mt-12 grid gap-8 lg:mt-16 lg:grid-cols-2 lg:gap-x-12 lg:gap-y-12">
            {c.massanfertigung.blocks.map((block, i) => (
