/*
 * /prothesen/fuss-prothese — Foot prosthesis sub-page.
 *
 * Mirrors sanimotion.com/fuss-prothese-berlin/: hero (split with photo) →
 * Prothetik von Sanimotion (split with detail image) →
 * Vor-Ort-Beratung & Außendienst → Was uns auszeichnet (6 highlights) →
 * contact.
 */
"use client";
import type { Route } from "./+types/prothesen.fuss-prothese";
...
import { getFussProtheseContent } from "~/content/pages/fuss-prothese";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/fuss-prothese/fuss-prothese-hero.png";
import detailImageUrl from "~/images/fuss-prothese/fuss-prothese-detail.png";

export function meta() {
  const c = getFussProtheseContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/prothesen/fuss-prothese" });
}

export default function FussProthese() {
  const locale = useLocale();
  const c = getFussProtheseContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Fuß-Prothese", path: "/prothesen/fuss-prothese" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/prothesen/fuss-prothese" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Prothesen", path: "/prothesen" },
        { name: "Fuß-Prothese", path: "/prothesen/fuss-prothese" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Fußprothese aus dem Sanimotion Sanitätshaus Berlin"
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Maßgefertigte Fußprothese – aus der Sanimotion-Werkstatt in Berlin-Kreuzberg"
