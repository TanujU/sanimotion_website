/*
 * /orthesen/daumen-orthese — Thumb orthosis sub-page.
 *
 * Mirrors sanimotion.com/daumen-orthese-berlin/: hero (split with photo) →
 * Vorkonfektionierte Daumenorthesen → Sonderanfertigungen block with detail
 * photo → Was uns auszeichnet (6 highlights) → FAQ accordion → contact.
 */
"use client";
import type { Route } from "./+types/orthesen.daumen-orthese";
...
import { getDaumenOrtheseContent } from "~/content/pages/daumen-orthese";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/daumen-orthese/daumen-orthese-hero.png";
import detailImageUrl from "~/images/daumen-orthese/daumen-orthese-detail.png";

export function meta() {
  const c = getDaumenOrtheseContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/orthesen/daumen-orthese" });
}

export default function DaumenOrthese() {
  const locale = useLocale();
  const c = getDaumenOrtheseContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Daumen-Orthese", path: "/orthesen/daumen-orthese" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/orthesen/daumen-orthese" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Orthesen", path: "/orthesen" },
        { name: "Daumen-Orthese", path: "/orthesen/daumen-orthese" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Daumenorthese aus dem Sanimotion Sanitätshaus Berlin"
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Maßgefertigte Daumenorthese aus der Sanimotion-Werkstatt in Berlin-Kreuzberg"
