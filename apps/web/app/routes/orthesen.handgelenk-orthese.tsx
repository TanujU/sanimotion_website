/*
 * /orthesen/handgelenk-orthese — Wrist orthosis sub-page.
 *
 * Mirrors sanimotion.com/handgelenk-orthese-berlin/: hero (split with photo) →
 * Vorkonfektionierte Handgelenkorthesen → Sonderanfertigungen block with detail
 * photo → Was uns auszeichnet (6 highlights) → FAQ accordion → contact.
 */
"use client";
import type { Route } from "./+types/orthesen.handgelenk-orthese";
...
import { getHandgelenkOrtheseContent } from "~/content/pages/handgelenk-orthese";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/handgelenk-orthese/handgelenk-orthese-hero.png";
import detailImageUrl from "~/images/handgelenk-orthese/handgelenk-orthese-detail.png";

export function meta() {
  const c = getHandgelenkOrtheseContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/orthesen/handgelenk-orthese" });
}

export default function HandgelenkOrthese() {
  const locale = useLocale();
  const c = getHandgelenkOrtheseContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Handgelenk-Orthese", path: "/orthesen/handgelenk-orthese" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/orthesen/handgelenk-orthese" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Orthesen", path: "/orthesen" },
        { name: "Handgelenk-Orthese", path: "/orthesen/handgelenk-orthese" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Handgelenkorthese aus dem Sanimotion Sanitätshaus Berlin"
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Maßgefertigte Handgelenkorthese aus der Sanimotion-Werkstatt in Berlin-Kreuzberg"
