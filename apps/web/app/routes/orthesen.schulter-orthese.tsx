/*
 * /orthesen/schulter-orthese — Shoulder orthosis sub-page.
 *
 * Mirrors sanimotion.com/schulter-orthese-berlin/: hero (split with photo) →
 * Vorkonfektionierte Schulterorthesen → Sonderanfertigungen block with detail
 * photo → Was uns auszeichnet (6 highlights) → FAQ accordion → contact.
 */
"use client";
import type { Route } from "./+types/orthesen.schulter-orthese";
...
import { getSchulterOrtheseContent } from "~/content/pages/schulter-orthese";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/schulter-orthese/schulter-orthese-hero.png";
import detailImageUrl from "~/images/schulter-orthese/schulter-orthese-detail.png";

export function meta() {
  const c = getSchulterOrtheseContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/orthesen/schulter-orthese" });
}

export default function SchulterOrthese() {
  const locale = useLocale();
  const c = getSchulterOrtheseContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Schulter-Orthese", path: "/orthesen/schulter-orthese" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/orthesen/schulter-orthese" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Orthesen", path: "/orthesen" },
        { name: "Schulter-Orthese", path: "/orthesen/schulter-orthese" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Schulterorthese aus dem Sanimotion Sanitätshaus Berlin"
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Maßgefertigte Schulterorthese aus der Sanimotion-Werkstatt in Berlin-Kreuzberg"
