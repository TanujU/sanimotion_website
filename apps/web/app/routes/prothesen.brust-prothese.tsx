/*
 * /prothesen/brust-prothese — Breast prosthesis sub-page.
 *
 * Mirrors sanimotion.com/brust-prothese-berlin/: hero (split with photo) →
 * Brustprothetik im Sanimotion Sanitätshaus (split with detail image) →
 * Top-Service / Vor-Ort-Beratung → Was uns auszeichnet (6 highlights) →
 * contact.
 */
"use client";
import type { Route } from "./+types/prothesen.brust-prothese";
...
import { getBrustProtheseContent } from "~/content/pages/brust-prothese";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/brust-prothese/brust-prothese-hero.jpg";
import detailImageUrl from "~/images/brust-prothese/brust-prothese-detail.jpg";

export function meta() {
  const c = getBrustProtheseContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/prothesen/brust-prothese" });
}

export default function BrustProthese() {
  const locale = useLocale();
  const c = getBrustProtheseContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Brust-Prothese", path: "/prothesen/brust-prothese" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/prothesen/brust-prothese" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Prothesen", path: "/prothesen" },
        { name: "Brust-Prothese", path: "/prothesen/brust-prothese" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Brustprothese aus dem Sanimotion Sanitätshaus Berlin"
      ...
      {/* Brustprothetik — split with detail photo */}
      <Section tone="muted">
        <Container>
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Silikon-Brustprothese mit passendem BH – Sanimotion Sanitätshaus Berlin"
