/*
 * /orthesen/ruecken-orthese — Back orthosis sub-page.
 *
 * Mirrors sanimotion.com/ruecken-orthese-berlin/: hero (split with photo) →
 * Vorkonfektionierte Rückenorthesen → Sonderanfertigungen block with detail
 * photo → Was uns auszeichnet (6 highlights) → FAQ accordion → contact.
 */
"use client";
import type { Route } from "./+types/orthesen.ruecken-orthese";
...
import { getRueckenOrtheseContent } from "~/content/pages/ruecken-orthese";
import { useLocale } from "~/i18n/locale";
import heroImageUrl from "~/images/ruecken-orthese/ruecken-orthese-hero.gif";
import detailImageUrl from "~/images/ruecken-orthese/ruecken-orthese-detail.png";

export function meta() {
  const c = getRueckenOrtheseContent("de");
  return buildMeta({ title: c.meta.title, description: c.meta.description, path: "/orthesen/ruecken-orthese" });
}

export default function RueckenOrthese() {
  const locale = useLocale();
  const c = getRueckenOrtheseContent(locale);
  ...
  return (
    <>
      <JsonLd schema={buildBreadcrumbSchema([
        ...
        { name: "Rücken-Orthese", path: "/orthesen/ruecken-orthese" },
      ])} />
      <JsonLd schema={buildMedicalDeviceSchema({ name: c.meta.title, description: c.meta.description, path: "/orthesen/ruecken-orthese" })} />
      <Breadcrumb items={[
        { name: "Startseite", path: "/" },
        { name: "Orthesen", path: "/orthesen" },
        { name: "Rücken-Orthese", path: "/orthesen/ruecken-orthese" },
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-x-16">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-card bg-muted shadow-soft relative mx-auto aspect-square w-full max-w-lg overflow-hidden">
                <img
                  src={heroImageUrl}
                  alt="Rückenorthese aus dem Sanimotion Sanitätshaus Berlin"
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Maßgefertigte Rückenorthese aus der Sanimotion-Werkstatt in Berlin-Kreuzberg"
