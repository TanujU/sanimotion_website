/*
 * /orthopaedische-schuhe — Orthopedic shoes landing page.
 *
 * Mirrors the structure of sanimotion.com/orthopaedische-schuhe-berlin/:
 * hero (split with photo) → intro story → workshop / physician collab →
 * 6 service highlights → Krankenkasse → Fußvermessung (with photo) →
 * Laufanalyse → Design → contact details + map (reused from home).
 */
"use client";
...
import { getSchuheContent } from "~/content/pages/orthopaedische-schuhe";
...
export default function OrthopaedischeSchuhe() {
  ...
  const c = getSchuheContent(locale);
  ...
  return (
    <>
      ...
      <Section tone="muted">
        <Container>
          <div className="grid gap-12 lg:grid-cols-12 lg:gap-16">
            <div className="lg:col-span-5">
              <Reveal>
                <Eyebrow>{c.collaboration.eyebrow}</Eyebrow>
                <Heading
                  as="h2"
                  size="display-md"
            ...
            <div className="lg:col-span-7">
              <div className="space-y-6">
                {c.collaboration.paragraphs.map((p, i) => (
      ...
      {/* Fußvermessung — with detail photo */}
      <Section tone="canvas">
        <Container>
            ...
            <div className="lg:col-span-5">
              <Reveal>
                <div className="rounded-card bg-muted shadow-soft relative aspect-[4/5] overflow-hidden">
                  <img
                    src={detailImageUrl}
                    alt="Fußvermessung im Sanimotion-Sanitätshaus"
                    className="ease-apple absolute inset-0 h-full w-full object-cover transition-transform duration-700 hover:scale-[1.03]"
                    loading="lazy"
                    decoding="async"
                  />
